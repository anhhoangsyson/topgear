...
export default function BenefitCard({ title, description }: BenefitCardProps) {
  return (
    <div className="bg-gray-100 p-4 sm:p-6 md:p-8 rounded-xl h-full">
      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-200 flex justify-center items-center mx-auto">
        <svg
          width="24"
          height="22"
          viewBox="0 0 24 22"
      ...
      <div>
        <h2 className="mt-3 text-base sm:text-lg md:text-xl font-semibold text-center">{title}</h2>
        <p className="mt-3 text-sm sm:text-base text-gray-400 text-center">{description}</p>
      </div>
    </div>
  );
}
