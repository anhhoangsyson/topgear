      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="all" className="flex items-center justify-center gap-2 text-xs sm:text-sm">
            ...
            <span className="hidden sm:inline">Tất cả ({isNaN(totalCounts.all) ? filteredNotifications.length : totalCounts.all})</span>
            <span className="sm:hidden">Tất cả</span>
          </TabsTrigger>
          <TabsTrigger value="unread" className="flex items-center justify-center gap-2 text-xs sm:text-sm">
            <BellOff className="h-4 w-4" />
            <span className="hidden sm:inline">Chưa đọc ({isNaN(totalCounts.unread) ? unreadCount : totalCounts.unread})</span>
          ...
          <TabsTrigger value="read" className="flex items-center justify-center gap-2 text-xs sm:text-sm">
            <Bell className="h-4 w-4" />
            <span className="hidden sm:inline">Đã đọc ({isNaN(totalCounts.read) ? 0 : totalCounts.read})</span>
            <span className="sm:hidden">Đã đọc</span>
          </TabsTrigger>
        </TabsList>

        <TabsContent value="all" className="mt-6">
          <NotificationList notifications={filteredNotifications} isLoading={isLoading} emptyMessage="Không có thông báo nào" />
          {hasMore && filteredNotifications.length > 0 && (
            <div className="mt-6 text-center">
              <Button variant="outline" onClick={loadMore} disabled={isLoading}>Tải thêm</Button>
            </div>
          )}
        </TabsContent>

        <TabsContent value="unread" className="mt-6">
          <NotificationList notifications={filteredNotifications} isLoading={isLoading} emptyMessage="Không có thông báo chưa đọc" />
          {hasMore && filteredNotifications.length > 0 && (
            <div className="mt-6 text-center">
              <Button variant="outline" onClick={loadMore} disabled={isLoading}>Tải thêm</Button>
            </div>
        ...
        <TabsContent value="read" className="mt-6">
          <NotificationList notifications={filteredNotifications} isLoading={isLoading} emptyMessage="Không có thông báo đã đọc" />
          {hasMore && filteredNotifications.length > 0 && (
            <div className="mt-6 text-center">
              <Button variant="outline" onClick={loadMore} disabled={isLoading}>Tải thêm</Button>
            </div>
